...
def send_file(filename):
    with socket.socket(socket.AF_INET, socket.SOCK_DGRAM) as udp_socket:
        ...
        window = []  # Stores packet sequence IDs within the window
        file_data = [] # Stores chunks of file data

        # Read the entire file into memory (packetized)
        ...
        while base < total_packets:
            ...
            while seq_id < base + WINDOW_SIZE and seq_id < total_packets:
                packet = create_packet(seq_id, file_data[seq_id])
                udp_socket.sendto(packet, SERVER_ADDRESS)
                
                if transmission_start_time is None:
                    transmission_start_time = time.time()  # Start time at first transmission
                
                sent_packets[seq_id] = time.time()  # Track send time for packet delay calculation
                window.append(seq_id)
                seq_id += 1

            # Wait for ACKs and manage window
            try:
                while window:
                    ...
                    while window and window[0] < ack_id:
                        ...
                        if ack_seq in sent_packets:
                            packet_delays += (time.time() - sent_packets.pop(ack_seq))  # Calculate delay
                        base += 1  # Move window forward

            except socket.timeout:
                print("Timeout, retransmitting window...")
                for seq in window:
                    udp_socket.sendto(create_packet(seq, file_data[seq]), SERVER_ADDRESS)
                    sent_packets[seq] = time.time()  # Update retransmit time
