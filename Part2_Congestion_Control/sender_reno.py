...
cwnd = 1  # Congestion window (starts with 1 packet)
ssthresh = 64  # Slow start threshold
dup_acks = 0  # Counter for duplicate ACKs

def create_packet(seq_id, data):
    """Creates a packet with sequence ID and data."""
    return int.to_bytes(seq_id, SEQ_ID_SIZE, signed=True, byteorder='big') + data

def send_file(filename):
    global cwnd, ssthresh, dup_acks
    ...
    with socket.socket(socket.AF_INET, socket.SOCK_DGRAM) as udp_socket:
        ...
        seq_id = 0
        sent_packets = {}  # Stores packet send times
        window = []  # Stores packets currently in flight
        file_data = [] # Stores chunks of file data

        with open(filename, "rb") as f:
            while chunk := f.read(PACKET_SIZE - SEQ_ID_SIZE):
                file_data.append(chunk)

        total_packets = len(file_data)

        while base < total_packets:
            # Send packets up to congestion window size
            while seq_id < base + int(cwnd) and seq_id < total_packets:
                packet = create_packet(seq_id, file_data[seq_id])
                udp_socket.sendto(packet, SERVER_ADDRESS)

                if transmission_start_time is None:
                    transmission_start_time = time.time()  # Start timer on first send

                sent_packets[seq_id] = time.time()  # Track packet send time
                window.append(seq_id)
                seq_id += 1
            ...
            try:
                while window:
                    ...
                    if ack_id in sent_packets:
                        packet_delays += (time.time() - sent_packets.pop(ack_id))
                        packets_sent += 1

                    while window and window[0] < ack_id:
                        base += 1
                        window.pop()  # Move window forward

                    # TCP Reno: Handle congestion control
                    if ack_id == base:  # Expected ACK received
                        dup_acks = 0  # Reset duplicate ACK counter
                        if cwnd < ssthresh:
                            cwnd += 1  
                        else:
                            cwnd += 1 / cwnd  
                        ...
                        dup_acks += 1
                        if dup_acks == 3:  # Fast Retransmit triggered
                            udp_socket.sendto(create_packet(ack_id, file_data[ack_id]), SERVER_ADDRESS)
                            ssthresh = max(cwnd / 2, 1)  # Reduce threshold
        ...
        avg_packet_delay = packet_delays / packets_sent if packets_sent > 0 else 0
        performance_metric = 0.3 * (throughput / 1000) + 0.7 * avg_packet_delay
